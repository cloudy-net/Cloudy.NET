...
const FormField = ({ name, path, label, description, renderChrome, partial, settings, validators }) => {
    ...
    return html`<div class=${`mb-3 ${ Object.keys(validators).length ? 'needs-validation' : '' } ${ ValidationManager.isInvalidForPath(state.validationResults, path) ? 'was-validated' : '' } `}>
