...
function PendingChanges({ renderIf, onSelect, onClose }) {
    ...
    const getSubText = (state, contentType) => state.remove ? 'Slated for removal' : state.contentReference.keyValues ? `Changed fields: ${state.simpleChanges.length}` : `New ${contentType.lowerCaseName} (created ${state.referenceDate.toLocaleString()})`;
