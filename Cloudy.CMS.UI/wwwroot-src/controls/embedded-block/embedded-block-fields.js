const EmbeddedBlockFields = ({ type, path, dependencies }) => {
    ...
    return dependencies.html`<fieldset class="embedded-block"><legend class="embedded-block-type">${type}<//>${fields.map(field => dependencies.html`<${dependencies.FormField} ...${field} path=${`${path}.${field.name}`} dependencies=${dependencies} />`)}<//>`;
};
