            ...
            <div class="card-body">
                ${options.map((option, index) => dependencies.html`
                    <div class="form-check">
                        <input onChange=${onChange} checked=${currentValues.includes(option.value)} disabled=${option.disabled} class="form-check-input" type="checkbox" value="${option.value}" id="${dependencies.componentContextProvider.getIdentifier(path)}-${index}" />
                        <label class="form-check-label" for="${dependencies.componentContextProvider.getIdentifier(path)}-${index}">${option.text}</label>
                    </div>`)
                }
                ...
                ${Object.keys(optionGroups).map((optionGroup, optionGroupIndex) => dependencies.html`
                    ...
                    ${optionGroups[optionGroup].options.map((option, index) => dependencies.html`
                        <div class="form-check">
                            <input onChange=${onChange} checked=${currentValues.includes(option.value)} disabled=${option.disabled} class="form-check-input" type="checkbox" id="${dependencies.componentContextProvider.getIdentifier(path)}-${optionGroupIndex}-${index}" value=${option.value}>${option.text}</option>
                            <label class="form-check-label" for="${dependencies.componentContextProvider.getIdentifier(path)}-${optionGroupIndex}-${index}">${option.text}</label>
                        </div>
                    `)}
                `)}
            </div>
